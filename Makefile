...
up: check_pkgs
	...
	for dir in software/pkg_*/; do
	  if [[ "${skip_packages}" != *"$${dir:13:-1}"* ]]; then # if array doesn't contain x ; cut off first 13 and last 1 chars "software/pkg_xxx/" -> 'xxx'
	    composefiles="$${composefiles} -f $${dir}compose.yaml" # software/pkg_xxx/ -> software/pkg_xxx/compose.yaml
	  fi
	done
...
down: check_pkgs
	...
	for dir in software/pkg_*/; do
	  ...
	  if [[ "${skip_packages}" == *"$${pkg:4}"* ]]; then # cut off first 4 chars "pkg_" -> 'xxx'
	    echo -e "\033[7;95;1m   >>skipping $${dir}  \033[0m"
	  else
	    echo -e "\033[7;95;1m   >>stopping $${dir}  \033[0m"
...
stop: check_pkgs
	...
	for dir in software/pkg_*/; do
	  ...
	  if [[ "${skip_packages}" == *"$${pkg:4}"* ]]; then # cut off first 4 chars "pkg_" -> 'xxx'
	    ...
	    echo -e "\033[7;95;1m   >>starting $${dir}  \033[0m"
...
build: check_pkgs
	...
	for dir in software/pkg_*/; do
	  ...
	  if [[ "${skip_packages}" == *"$${pkg:4}"* ]]; then # cut off first 4 chars "pkg_" -> 'xxx'
	    ...
	    echo -e "\033[7;95;1m   >>building $${dir:9}  \033[0m"
	...
	for dir in software/pkg_*/; do
	  if [[ "${skip_packages}" != *"$${dir:13:-1}"* ]]; then # if array doesn't contain x ; cut off first 13 and last 1 chars "software/pkg_xxx/" -> 'xxx'
	    if [ ! -f "$$dir/compose.yaml" ]; then
	      echo -e "\033[7;91;1m   package $${dir:13:-1} missing compose.yaml  \033[0m "
	      exit 66
	    fi
